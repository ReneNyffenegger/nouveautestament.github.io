...
sebastien	= fs.readFileSync('../../database/sebastien_fr.txt', 'utf8').replace(/#/g,' ').split(/\r?\n/);
alain		= fs.readFileSync('../../database/dumont.txt', 'utf8').split(/\r?\n/);

//check 1
if (sebastien.length != alain.length)
	console.log('erreur 1')

for (nb=0;nb!=sebastien.length;nb++)
{
	
	//check 2
	if (sebastien[nb] != '' && alain[nb] != '')
	{
...
**/
		...
		textese = sebastien[nb].replace(se[0]+' ', "")
...
'+texteal+'</span>\
		...
		file2.appendFileSync(livre+'.html',body_htmlf+'\n', 'utf8');

		if (livre == 27 && chapitre == 22 && verset == 21)
		{
			file2.appendFileSync(livreb+'.html','</body></html>', 'utf8');
			file.writeFileSync('all.html',head_html+css_papier+body_html_all+'</table></body></html>', 'utf8');
		}
