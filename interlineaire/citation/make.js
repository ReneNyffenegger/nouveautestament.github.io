...
fichier1	= require('fs');
fichier2	= require('fs');
database_fr	= fichier1.readFileSync('../../database/database_ci.txt', 'utf8').split('\n');
...
//console.log(line+'--> '+backverset+' '+backlivre+'-'+backchap+' '+database_fr[line]+' '+livre+' '+chapitre)
if ((chapitre != backchap || livre != backlivre) && xxxxx != '')
{
...
intro2 = intro + chbook[livre]+'<br><br>----VERSET----';

intro2	=	intro2.replace('>'+nomdulivre+'<','><b>[ '+nomdulivre+' ]</b><');
intro2	=	intro2.replace('>'+'CH'+chapitre+'<','><b>[ CH'+chapitre+' ]</b><')
...
if (backverset != verset || (backverset == verset && backchap != chapitre))
{

	if (verset == 1)
		xxxxx +=`
<br><div id="V`+verset+`"></div><a href="#a"><b>`+xbook[livre]+' '+chapitre+' '+verset+`</b></a>&ensp;&ensp;<table cellspacing="0"><tbody>`;
...
//check + creation
for (livrecheck = 1 ; livrecheck != 28 ; livrecheck++)
{	
	for (chapcheck = 1 ; chapcheck != parseInt(nbch[livrecheck])+1 ; chapcheck++)
		{
			
			if (!fichier2.existsSync('./'+livrecheck+'-'+chapcheck+'.html'))
			{
				//console.log(livrecheck+'-'+chapcheck+'.html'+' NO')


				intro2 = intro + chbook[livrecheck];

				intro2	=	intro2.replace('>'+xbook[livrecheck]+'<','><b>[ '+xbook[livrecheck]+' ]</b><');
				intro2	=	intro2.replace('>'+'CH'+chapcheck+'<','><b>[ CH'+chapcheck+' ]</b><')


				fichier2.writeFileSync(livrecheck+'-'+chapcheck+'.html',intro2+'<br></body></html>', 'utf8');
