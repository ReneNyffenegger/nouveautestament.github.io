...
//for (line = 0 ; line != plusdeligne ; line++){
while (end == 0) {
...
if (sebastien_te[lineseb] != "" && sebastien_te[lineseb]) { 
	...
	lcvteinfo	= sebastien_te[lineseb].split(':');
	lcvte = convnumber( lcvteinfo[0] , lcvteinfo[1] , lcvteinfo[2] );
...
if (tauber_te[linejames] != "" && tauber_te[linejames]) { 
	...
	lcvtainfo	= tauber_te[linejames].split(':');
	lcvta		= convnumber( lcvtainfo[0] , lcvtainfo[1] , lcvtainfo[2] );
...
if (ieronymus_ro[linejerome] != "" && ieronymus_ro[linejerome]) { 
	...
	lcvroinfo	= ieronymus_ro[linejerome].split(':');
	lcvro		= convnumber( lcvroinfo[0] , lcvroinfo[1] , lcvroinfo[2]);  
...
if (sebastien_te[lineseb] != "" && sebastien_te[lineseb] && waitseb == 0) {
	...
	lcvteinfo	= sebastien_te[lineseb].split(' ')
	lcvfrinfo	= sebastien_fr[lineseb].split(' ')


	textete = sebastien_te[lineseb].replace(lcvteinfo[0]+' ',"").split(' ');
	textefr = sebastien_fr[lineseb].replace(lcvfrinfo[0]+' ',"").split('#');
...
if (ieronymus_ro[linejerome] != "" && ieronymus_ro[linejerome] && waitjerome == 0) {
	...
	if (textero != "")
	{
		ieronymus = '<div class="int">'+textero+'</div>';
	}
...
if (tauber_te[linejames] != "" && tauber_te[linejames] && waitjames == 0) {
	...
	texteta = tauber_te[linejames].replace(lcvtainfo[0]+' ',"").split(' ');
...
//CREATION TR SPAN
for (s = 0 ; s != texteta.length ; s++)
{
	if (texteta[s].split('=')[2].match(/,/))
	{
		lem1a = texteta[s].split('=')[2].split(',')[0];
		lem2a = texteta[s].split('=')[2].replace(lem1a+',',"");
	}

	else
	{
		lem1a = texteta[s].split('=')[2];
		lem2a = '-';
	}
...
james +=`
<div class="int">
<span class="el">`+texteta[s].split('=')[0]+`</span>
<span class="info">`+lem1a+`</span>
<span class="subinfo">`+lem2a+`</span>
<span class="lemme">`+texteta[s].split('=')[1]+`</span>
</div>
`
...
//check
//console.log(line+'--> '+backverset+' '+backlivre+'-'+backchap+' '+sebastien_te[line]+' '+livre+' '+chapitre)




//onchange chapitre or livre : add all-versets ; put to file ; reset xxxxx
...
<a href="7-1.html">1CORINTHIENS</a>&ensp;
<a href="8-1.html">2CORINTHIENS</a>&ensp;
...
<a href="13-1.html">1THESSALONICIENS</a>&ensp;
<a href="14-1.html">2THESSALONICIENS</a>&ensp;
<a href="15-1.html">1TIMOTHEE</a>&ensp;
<a href="16-1.html">2TIMOTHEE</a>&ensp;
...
<a href="21-1.html">1PIERRE</a>&ensp;
<a href="22-1.html">2PIERRE</a>&ensp;
<a href="23-1.html">1JEAN</a>&ensp;
<a href="24-1.html">2JEAN</a>&ensp;
<a href="25-1.html">3JEAN</a>&ensp;
<a href="26-1.html">JUDAS</a>&ensp;
<a href="27-1.html">APOCALYPSE</a><br><br>
`;

intro += chbook[livre]+'<br><br>----VERSET----';

intro	=	intro.replace('>'+nomdulivre+'<','><b>[ '+nomdulivre+' ]</b><');
intro	=	intro.replace('>'+'CH'+chapitre+'<','><b>[ CH'+chapitre+' ]</b><')
...
//AJOUTER
//&ensp;<a href="../comparer-grec/`+livre+`-`+chapitre+`.html#v1">HEL</a>
//&ensp;<a href="../comparer-interlineaire/`+livre+`-`+chapitre+`.html#v1">INT</a>




//change verset : add verset intro
...
//add tr 1
xxxxx += `
<tr><td class="td1">SEBASTIEN-D-G</td><td class="td2">2021</td><td class="td3">`+sebastien+`</td></tr>
`;
